# ListAccountingExpensesQueryParamFields

## Example Usage

```go
import (
	"github.com/unified-to/unified-go-sdk/pkg/models/operations"
)

value := operations.ListAccountingExpensesQueryParamFieldsID
```


## Values

| Name                                                     | Value                                                    |
| -------------------------------------------------------- | -------------------------------------------------------- |
| `ListAccountingExpensesQueryParamFieldsID`               | id                                                       |
| `ListAccountingExpensesQueryParamFieldsCreatedAt`        | created_at                                               |
| `ListAccountingExpensesQueryParamFieldsUpdatedAt`        | updated_at                                               |
| `ListAccountingExpensesQueryParamFieldsUserID`           | user_id                                                  |
| `ListAccountingExpensesQueryParamFieldsContactID`        | contact_id                                               |
| `ListAccountingExpensesQueryParamFieldsAccountID`        | account_id                                               |
| `ListAccountingExpensesQueryParamFieldsName`             | name                                                     |
| `ListAccountingExpensesQueryParamFieldsPaymentMethod`    | payment_method                                           |
| `ListAccountingExpensesQueryParamFieldsPostedAt`         | posted_at                                                |
| `ListAccountingExpensesQueryParamFieldsTotalAmount`      | total_amount                                             |
| `ListAccountingExpensesQueryParamFieldsCurrency`         | currency                                                 |
| `ListAccountingExpensesQueryParamFieldsTaxAmount`        | tax_amount                                               |
| `ListAccountingExpensesQueryParamFieldsReimbursedAmount` | reimbursed_amount                                        |
| `ListAccountingExpensesQueryParamFieldsReimbursedAt`     | reimbursed_at                                            |
| `ListAccountingExpensesQueryParamFieldsApprovedAt`       | approved_at                                              |
| `ListAccountingExpensesQueryParamFieldsApproverUserID`   | approver_user_id                                         |
| `ListAccountingExpensesQueryParamFieldsLineitems`        | lineitems                                                |
| `ListAccountingExpensesQueryParamFieldsAttachments`      | attachments                                              |
| `ListAccountingExpensesQueryParamFieldsOrganizationID`   | organization_id                                          |
| `ListAccountingExpensesQueryParamFieldsUsers`            | users                                                    |
| `ListAccountingExpensesQueryParamFieldsApproverUsers`    | approver_users                                           |
| `ListAccountingExpensesQueryParamFieldsStatus`           | status                                                   |
| `ListAccountingExpensesQueryParamFieldsExternalNumber`   | external_number                                          |
| `ListAccountingExpensesQueryParamFieldsCategoryIds`      | category_ids                                             |
| `ListAccountingExpensesQueryParamFieldsProjectID`        | project_id                                               |
| `ListAccountingExpensesQueryParamFieldsRaw`              | raw                                                      |