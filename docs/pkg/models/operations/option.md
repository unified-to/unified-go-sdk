...
```go
unifiedgosdk.WithServerURL("https://api.example.com")
```
...
```go
unifiedgosdk.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
unifiedgosdk.WithClient(httpClient)
```
...
```go
unifiedgosdk.WithSecuritySource(/* ... */)
```
...
```go
unifiedgosdk.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
unifiedgosdk.WithTimeout(30 * time.Second)
```
