# ListAccountingProfitlossesQueryParamFields

## Example Usage

```go
import (
	"github.com/unified-to/unified-go-sdk/pkg/models/operations"
)

value := operations.ListAccountingProfitlossesQueryParamFieldsID
```


## Values

| Name                                                                   | Value                                                                  |
| ---------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `ListAccountingProfitlossesQueryParamFieldsID`                         | id                                                                     |
| `ListAccountingProfitlossesQueryParamFieldsCreatedAt`                  | created_at                                                             |
| `ListAccountingProfitlossesQueryParamFieldsUpdatedAt`                  | updated_at                                                             |
| `ListAccountingProfitlossesQueryParamFieldsCategoryIds`                | category_ids                                                           |
| `ListAccountingProfitlossesQueryParamFieldsStartAt`                    | start_at                                                               |
| `ListAccountingProfitlossesQueryParamFieldsEndAt`                      | end_at                                                                 |
| `ListAccountingProfitlossesQueryParamFieldsName`                       | name                                                                   |
| `ListAccountingProfitlossesQueryParamFieldsCurrency`                   | currency                                                               |
| `ListAccountingProfitlossesQueryParamFieldsIncome`                     | income                                                                 |
| `ListAccountingProfitlossesQueryParamFieldsExpenses`                   | expenses                                                               |
| `ListAccountingProfitlossesQueryParamFieldsCostOfGoodsSold`            | cost_of_goods_sold                                                     |
| `ListAccountingProfitlossesQueryParamFieldsGrossProfitAmount`          | gross_profit_amount                                                    |
| `ListAccountingProfitlossesQueryParamFieldsNetProfitAmount`            | net_profit_amount                                                      |
| `ListAccountingProfitlossesQueryParamFieldsIncomeTotalAmount`          | income_total_amount                                                    |
| `ListAccountingProfitlossesQueryParamFieldsNetIncomeAmount`            | net_income_amount                                                      |
| `ListAccountingProfitlossesQueryParamFieldsExpensesTotalAmount`        | expenses_total_amount                                                  |
| `ListAccountingProfitlossesQueryParamFieldsCostOfGoodsSoldTotalAmount` | cost_of_goods_sold_total_amount                                        |
| `ListAccountingProfitlossesQueryParamFieldsIncomeSections`             | income_sections                                                        |
| `ListAccountingProfitlossesQueryParamFieldsExpensesSections`           | expenses_sections                                                      |
| `ListAccountingProfitlossesQueryParamFieldsCostOfGoodsSoldSections`    | cost_of_goods_sold_sections                                            |
| `ListAccountingProfitlossesQueryParamFieldsRaw`                        | raw                                                                    |