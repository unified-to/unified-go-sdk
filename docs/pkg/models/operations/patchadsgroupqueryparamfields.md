# PatchAdsGroupQueryParamFields

## Example Usage

```go
import (
	"github.com/unified-to/unified-go-sdk/pkg/models/operations"
)

value := operations.PatchAdsGroupQueryParamFieldsID
```


## Values

| Name                                                | Value                                               |
| --------------------------------------------------- | --------------------------------------------------- |
| `PatchAdsGroupQueryParamFieldsID`                   | id                                                  |
| `PatchAdsGroupQueryParamFieldsCreatedAt`            | created_at                                          |
| `PatchAdsGroupQueryParamFieldsUpdatedAt`            | updated_at                                          |
| `PatchAdsGroupQueryParamFieldsName`                 | name                                                |
| `PatchAdsGroupQueryParamFieldsParentID`             | parent_id                                           |
| `PatchAdsGroupQueryParamFieldsCampaignID`           | campaign_id                                         |
| `PatchAdsGroupQueryParamFieldsOrganizationID`       | organization_id                                     |
| `PatchAdsGroupQueryParamFieldsInsertionorderID`     | insertionorder_id                                   |
| `PatchAdsGroupQueryParamFieldsStatus`               | status                                              |
| `PatchAdsGroupQueryParamFieldsTargeting`            | targeting                                           |
| `PatchAdsGroupQueryParamFieldsBidAmount`            | bid_amount                                          |
| `PatchAdsGroupQueryParamFieldsBidStrategy`          | bid_strategy                                        |
| `PatchAdsGroupQueryParamFieldsBudgetAmount`         | budget_amount                                       |
| `PatchAdsGroupQueryParamFieldsBudgetPeriod`         | budget_period                                       |
| `PatchAdsGroupQueryParamFieldsBudgetAllocationType` | budget_allocation_type                              |
| `PatchAdsGroupQueryParamFieldsStartAt`              | start_at                                            |
| `PatchAdsGroupQueryParamFieldsEndAt`                | end_at                                              |
| `PatchAdsGroupQueryParamFieldsBudgetUnit`           | budget_unit                                         |
| `PatchAdsGroupQueryParamFieldsBudgetMaxAmount`      | budget_max_amount                                   |
| `PatchAdsGroupQueryParamFieldsType`                 | type                                                |
| `PatchAdsGroupQueryParamFieldsHasEuPoliticalAds`    | has_eu_political_ads                                |
| `PatchAdsGroupQueryParamFieldsPacing`               | pacing                                              |
| `PatchAdsGroupQueryParamFieldsFrequencyCap`         | frequency_cap                                       |
| `PatchAdsGroupQueryParamFieldsCreativeIds`          | creative_ids                                        |
| `PatchAdsGroupQueryParamFieldsOptimizationGoal`     | optimization_goal                                   |
| `PatchAdsGroupQueryParamFieldsBillingEvent`         | billing_event                                       |
| `PatchAdsGroupQueryParamFieldsDailySpendCap`        | daily_spend_cap                                     |
| `PatchAdsGroupQueryParamFieldsLifetimeSpendCap`     | lifetime_spend_cap                                  |
| `PatchAdsGroupQueryParamFieldsAdGroupType`          | ad_group_type                                       |
| `PatchAdsGroupQueryParamFieldsCurrency`             | currency                                            |
| `PatchAdsGroupQueryParamFieldsMetadata`             | metadata                                            |
| `PatchAdsGroupQueryParamFieldsRaw`                  | raw                                                 |