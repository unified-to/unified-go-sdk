// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package shared

// PropertyMessagingMessageAuthorMember - for email systems, this field represents the From value
type PropertyMessagingMessageAuthorMember struct {
	Email    *string `json:"email,omitempty"`
	ImageURL *string `json:"image_url,omitempty"`
	Name     *string `json:"name,omitempty"`
	UserID   *string `json:"user_id,omitempty"`
}

func (p *PropertyMessagingMessageAuthorMember) GetEmail() *string {
	if p == nil {
		return nil
	}
	return p.Email
}

func (p *PropertyMessagingMessageAuthorMember) GetImageURL() *string {
	if p == nil {
		return nil
	}
	return p.ImageURL
}

func (p *PropertyMessagingMessageAuthorMember) GetName() *string {
	if p == nil {
		return nil
	}
	return p.Name
}

func (p *PropertyMessagingMessageAuthorMember) GetUserID() *string {
	if p == nil {
		return nil
	}
	return p.UserID
}
