...
func (s *Unified) ListUnifiedSupports(ctx context.Context) (*operations.ListUnifiedSupportsResponse, error) {
	hookCtx := hooks.HookContext{
		Context:        ctx,
		OperationID:    "listUnifiedSupports",
		SecuritySource: s.sdkConfiguration.Security,
	}

	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	opURL, err := url.JoinPath(baseURL, "/unified/support")
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.ListUnifiedSupportsResponse{
		StatusCode:  httpRes.StatusCode,
		ContentType: httpRes.Header.Get("Content-Type"),
		RawResponse: httpRes,
	}
